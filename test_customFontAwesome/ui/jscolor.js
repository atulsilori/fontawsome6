...
var jsc = {
	...
	jscolor: function (targetElement, options) {
		...
		this.toRGBString = function () {
			return (
				'rgb(' + Math.round(this.rgb[0]) + ',' + Math.round(this.rgb[1]) + ',' + Math.round(this.rgb[2]) + ')'
			);
		};
		...
		function drawPicker() {
			...
			p.sldPtrIB.style.setProperty(
				'border',
				THIS.pointerBorderWidth + 'px solid ' + THIS.pointerBorderColor,
				'important',
			);
			p.sldPtrOB.style.setProperty(
				'border',
				THIS.pointerBorderWidth + 'px solid ' + THIS.pointerBorderColor,
				'important',
			);
		...
		function redrawPad() {
			...
			switch (jsc.getSliderComponent(THIS)) {
				case 's':
					...
					var color1 =
						'rgb(' + Math.round(rgb1[0]) + ',' + Math.round(rgb1[1]) + ',' + Math.round(rgb1[2]) + ')';
					var color2 =
						'rgb(' + Math.round(rgb2[0]) + ',' + Math.round(rgb2[1]) + ',' + Math.round(rgb2[2]) + ')';
